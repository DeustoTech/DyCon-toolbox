...
%U0_tline = [2.3*ones([length(tline),M]),5*ones([length(tline),1])];
%U0_tline = [repmat((2.7-1.05*tline').*(tline'>1/10),[1 M])];%,5*ones([length(tline),1])];
%U0_tline = [UO_tline,1*ones([length(tline),1])];
U0_tline = [repmat(2.8*ones(size(tline')),[1 M])];%,5*ones([length(tline),1])];
u_f = [0;0];
...
zz = YO_tline;
tline_UO = tline; % timeline based on the values of t, which is the integration of T(s)ds.
% Cost calcultaion
Final_Time = tline_UO(end);
