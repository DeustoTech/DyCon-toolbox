...
U0_tline = [-1*ones([length(tline),1]),1*ones([length(tline),1]),8*ones([length(tline),1])];
...
j=floor(6/Final_Time/dt);
plot(zz(j,1),zz(j,2),'bo');
plot(zz(j,9),zz(j,10),'ko');
plot(zz(j,3),zz(j,4),'ro');

j=floor(8/Final_Time/dt);
plot(zz(j,1),zz(j,2),'bo');
plot(zz(j,9),zz(j,10),'ko');
plot(zz(j,3),zz(j,4),'ro');
...
%temp = temp3;
%temp = try1;
GradientMethod(iP,'DescentAlgorithm',@ConjugateGradientDescent,'DescentParameters',{'DirectionParameter','PPR'},'tol',1e-6,'Graphs',true,'U0',temp);
%GradientMethod(iP,'DescentAlgorithm',@AdaptativeDescent,'Graphs',true,'U0',temp);
%plot(iP)
temp = iP.solution.UOptimal;
%%
