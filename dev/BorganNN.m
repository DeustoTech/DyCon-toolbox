...
% Generate data
N = 100;
InXdata = linspace(-1,1,N);
InYdata = linspace(-1,1,N);

OutFcn = @(x,y) double((x.^2 + y.^2) < 0.5);
%
Outdata = OutFcn(InXdata,InYdata);
%
%% Only take some data
indsx = randsample(1:N,floor(N/2),false);
indsy = randsample(1:N,floor(N/2),false);

InXdata_train = InXdata(indsx);
InYdata_train = InYdata(indsy);
Outdata_train = OutFcn(InXdata_train,InYdata_train);
%%
clf
[xms,yms] = meshgrid(InXdata,InYdata);
%surf(xms,yms,OutFcn(xms,yms));
view(0,-90)
hold on
shading interp
scatter3(InXdata_train,InYdata_train,Outdata_train,[],Outdata_train)
%%
f = figure(1)
