...
blue = [0 0 1];
sblue = 0.5*blue + 0.5*[1 1 1]; 

red = [1 0 0];
sred = 0.5*red + 0.5*[1 1 1]; 
...
%%
for k = 1:M_e
	line_sheep(k) = line(ue(1,1,k),ue(2,1,k),'LineStyle','-','LineWidth',4,'Parent',ax,'Color',red);
end
for k = 1:M_d
	line_dog(k) =line(ud(1,1,k),ud(2,1,k),'LineStyle','-','LineWidth',4,'Parent',ax,'Color',blue);
end


for k = 1:M_d
	point_dog(k) = line(ud(1,1,k),ud(2,1,k),'LineStyle','none','Marker','o','MarkerSize',12,'Parent',ax,'Color',blue,'MarkerFaceColor',sblue);
end

for k = 1:M_e
	point_sheep(k) = line(ue(1,1,k),ue(2,1,k),'LineStyle','none','Marker','o','MarkerSize',12,'Parent',ax,'Color',red,'MarkerFaceColor',sred);
end
...
pause(0.5)
    for k = 1:M_e
        line_sheep(k).XData = ue(1,:,k);
        line_sheep(k).YData =  ue(2,:,k);
    end
    
    for k = 1:M_d
        line_dog(k).XData = ud(1,:,k);
        line_dog(k).YData =  ud(2,:,k);
