function examples2blog(name,varargin)
     ...
     addOptional(p,'path_documentation','/home/djoroya/Documentos/GitHub/CCM/Webs/dycon-toolbox-documentation/posts')
     
     parse(p,name,varargin{:})
    
    path_documentation = p.Results.path_documentation;
    file        = 'StartDyconPlatform.m';
    pathtoolbox    =  replace(which(file),file,''); 
%%
    ...
    path_destination = [path_documentation,replace(path_folder,[pathtoolbox,'docs'],''),'.md'];
    ...
    OUTMD = fopen(path_destination,'w');
    fprintf(OUTMD,'---\n');
    ...
    if exist([path_folder,'/imgs-matlab'],'dir')
        ...
        system(['rm -r ',path_folder,'/imgs-matlab/'])
    end
