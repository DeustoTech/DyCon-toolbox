...
%    dimension: [1x1]
    ...
    iCP.hamiltonian = symL + symP.'*iCP.ode.Dynamic.Symbolic;
    % Para cada cordenada de U, calculamos la derivada de dH/du_i
    dH_du = arrayfun(@(u) diff(formula(iCP.hamiltonian),u), symU.');
    %
    iCP.gradient.sym = symfun(dH_du,[t symY.' symP.' symU.']);
    % Pasamos esta funcion a una function_handle
    iCP.gradient.num = matlabFunction(dH_du,'Vars',{t,symY,symP,symU});
